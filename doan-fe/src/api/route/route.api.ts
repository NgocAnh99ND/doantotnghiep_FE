...
const ENDPOINT = {
  ...
  // ✅ bạn đổi đúng endpoint BE của thuật toán ở đây
...
export const routeApi = {
  ...
  // ✅ NEW: gợi ý tuyến theo thuật toán BE
  async searchSuggested(params: { start: string; end: string }): Promise<RouteDTO[]> {
    const url =
      `${ENDPOINT.suggest}?start=${encodeURIComponent(params.start)}&end=${encodeURIComponent(params.end)}`;
    const res = await getJson<RoutesResponse>(url);
    return res?.data ?? [];
  },
};
