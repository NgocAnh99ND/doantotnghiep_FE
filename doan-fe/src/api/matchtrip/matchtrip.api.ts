...
export function ensureListOk<T>(res: ListResponse<T>): asserts res is { success: true; total: number; data: T[] } {
...
export const matchTripApi = {
  ...
  async fetchByPassenger(passenger_id: number): Promise<MatchTripItem[]> {
    const res = await getJson<ListResponse<MatchTripItem>>(`/api/passenger/matches?passenger_id=${passenger_id}`);
    ensureListOk(res);
    return res.data;
  },

  async fetchByRoute(route_id: number): Promise<MatchTripItem[]> {
    const res = await getJson<ListResponse<MatchTripItem>>(`/api/route/matches?route_id=${route_id}`);
    ensureListOk(res);
    return res.data;
  },
  ...
  async fetchPendingByDriver(driver_id: number): Promise<DriverPendingRequestItem[]> {
    const res = await getJson<ListResponse<DriverPendingRequestItem>>(`/api/driver/requests?driver_id=${driver_id}`);
    ensureListOk(res);
    return res.data;
  },

  async fetchAcceptedByDriver(driver_id: number): Promise<DriverAcceptedMatchItem[]> {
    const res = await getJson<ListResponse<DriverAcceptedMatchItem>>(`/api/driver/matches?driver_id=${driver_id}`);
    ensureListOk(res);
    return res.data;
  },

  async finishMatch(match_id: number): Promise<void> {
    // PUT /api/match-trip/finish  body: { match_id }
